...
class BaseModel():
    ...
    def save_img(self, sample, img_file):
        sample = torch.clamp((sample.detach() / 2) + 0.5, min=0, max=1)
        save_image(sample, img_file)
    ...
    def save_results(self, save_data, score=None, data_name='none'):
        ...
        result_save_dir = os.path.join(self.opt.results_dir, self.opt.name)
        result_save_dir = os.path.join(result_save_dir, self.opt.which_iter)
        ...
        for i in range(save_data.size(0)):
            ...
            util.mkdir(result_save_dir)
            img_path = os.path.join(result_save_dir, img_name)
